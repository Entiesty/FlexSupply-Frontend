<template>
  <div class="workspace-container">
    ...
    <div class="table-wrapper">
      ...
      >
        <el-table-column label="网点属性" width="140" align="center">
          <template #default="scope">
            <div class="biz-badge" :class="scope.row.isEmergencyHub === 1 ? 'req-type' : 'don-type'">
              {{ scope.row.isEmergencyHub === 1 ? '🚨 应急枢纽' : '🟢 普通网点' }}
            </div>
          </template>
        </el-table-column>

        <el-table-column label="据点核心信息" min-width="350" align="left">
          <template #default="{ row }">
            <div class="goods-info-cell">
              <div class="goods-main">
                <span class="g-name" :title="row.stationName">{{ row.stationName }}</span>
                <span v-if="row.hasFreezer === 1" class="freezer-tag">🧊 冷链级</span>
              </div>
              <div class="goods-sub" style="justify-content: flex-start; margin-top:4px;">
        ...
        <el-table-column prop="managerId" label="管理负责人" width="160" align="center">
          <template #default="scope">
            <span class="manager-badge" :class="{'unassigned': !scope.row.managerId}">
              <el-icon><User /></el-icon> {{ getManagerName(scope.row.managerId) }}
            </span>
          </template>
        </el-table-column>
      ...
      <div class="drawer-body" v-loading="inventoryLoading">
        <el-table :data="inventoryList" style="width: 100%" class="custom-table" empty-text="当前驿站大仓为空，等待志愿者送达物资">
          ...
          <el-table-column label="当前结余库存" width="120" align="center">
            <template #default="{ row }">
              <span style="font-size: 1.2rem; font-weight: 900; color: #3b82f6;">{{ row.stock }}</span>
            </template>
          </el-table-column>
          ...
          <div class="location-picker-box">
            <div class="loc-display" :class="{'has-val': form.longitude}">
              ...
              <div class="loc-coords" v-if="form.longitude">
                Lon: {{ form.longitude }} | Lat: {{ form.latitude }}
              </div>
            </div>
            <button type="button" class="pick-map-btn" @click="openMapDialog">
              <el-icon><MapLocation /></el-icon> 智能选点
            </button>
          </div>
        </el-form-item>
        ...
        <el-form-item label="指定负责人">
          <el-select v-model="form.managerId" filterable placeholder="请搜索并选择站长 (可输入姓名/手机号)" style="width: 100%">
            <el-option v-for="user in managerOptions" :key="user.userId" :label="user.username + ' (' + user.phone + ')'" :value="user.userId">
              <span style="float: left; font-weight: bold;">{{ user.username }}</span>
              <span style="float: right; color: #8492a6; font-size: 13px; font-family: monospace;">{{ user.phone }}</span>
            </el-option>
          </el-select>
        </el-form-item>
      </el-form>
...
import { Search, OfficeBuilding, Plus, Location, User, Setting, MapLocation, WarningFilled, CircleCheckFilled } from '@element-plus/icons-vue'
...
const form = reactive({ stationId: null, stationName: '', address: '', longitude: '', latitude: '', isEmergencyHub: 0, hasFreezer: 0, managerId: '' })
...
const fetchManagerOptions = async () => {
  try {
    const res = await getEligibleManagers()
...
const openAddDialog = () => {
  ...
  Object.assign(form, { stationId: null, stationName: '', address: '', longitude: '', latitude: '', isEmergencyHub: 0, hasFreezer: 0, managerId: '' })
...
const openEditDialog = (row) => {
  ...
  Object.assign(form, {
    ...
    hasFreezer: row.hasFreezer, managerId: row.managerId
  })
  dialogVisible.value = true
}
...
const submitAdd = async () => {
  ...
  submitting.value = true
  try {
    const payload = { ...form, longitude: parseFloat(form.longitude), latitude: parseFloat(form.latitude), managerId: form.managerId ? parseInt(form.managerId) : null }
    if (isEditMode.value) {
...
const getManagerName = (id) => {
  if (!id) return '尚未指派'
  const user = managerOptions.value.find(u => u.userId === id)
  return user ? user.username : `未知人员(#${id})`
}
...
const initMap = () => { window._AMapSecurityConfig = { securityJsCode: import.meta.env.VITE_AMAP_SECURITY_CODE }; AMapLoader.load({ key: import.meta.env.VITE_AMAP_KEY, version: "2.0", plugins: ['AMap.Geocoder', 'AMap.AutoComplete'] }).then((AMap) => { const centerPoint = (tempLoc.lng && tempLoc.lat) ? [tempLoc.lng, tempLoc.lat] : [118.833400, 24.980600]; mapInstance = new AMap.Map("amap-container", { viewMode: "2D", zoom: 15, center: centerPoint }); geocoderInstance = new AMap.Geocoder({ radius: 1000, extensions: "all" }); autoCompleteInstance = new AMap.AutoComplete({}); if (tempLoc.lng) { markerInstance = new AMap.Marker({ position: centerPoint }); mapInstance.add(markerInstance) } mapInstance.on('click', (e) => { const lng = e.lnglat.getLng(), lat = e.lnglat.getLat(); geocoderInstance.getAddress([lng, lat], (status, result) => { let addr = '该区域暂无详细街道信息'; if (status === 'complete' && result.info === 'OK') addr = result.regeocode.formattedAddress; updateMapByLocation(lng, lat, addr) }) }) }).catch(e => { console.error(e); ElMessage.error('地图引擎初始化失败') }) }
...
.freezer-tag { background: #f0fdf4; color: #166534; padding: 3px 8px; border-radius: 8px; font-weight: 900; font-size: 0.75rem; border: 1px solid #dcfce7; flex-shrink: 0;}
...
/* =========== 其他列元素 =========== */
.biz-badge { display: inline-block; padding: 5px 12px; border-radius: 8px; font-size: 0.8rem; font-weight: 900; letter-spacing: 1px; }
.don-type { background: #f8fafc; color: #475569; border: 1px solid #e2e8f0; }
.req-type { background: #fef2f2; color: #ef4444; border: 1px solid #fecaca; }

.manager-badge { display: inline-flex; align-items: center; gap: 6px; background: #fff7ed; color: #ea580c; padding: 6px 12px; border-radius: 8px; font-weight: 900; font-size: 0.85rem; border: 1px solid #ffedd5;}
.manager-badge.unassigned { background: #f8fafc; color: #94a3b8; border: 1px dashed #cbd5e1; font-weight: 500;}

.inventory-btn { font-weight: 900; font-size: 1rem; margin-right: 10px;}
...
.pick-map-btn { display: flex; align-items: center; justify-content: center; gap: 6px; background: #fff; border: 2px solid #3b82f6; color: #3b82f6; font-weight: 900; border-radius: 12px; padding: 0 15px; cursor: pointer; transition: 0.2s; white-space: nowrap; box-shadow: 0 4px 10px rgba(59,130,246,0.1); }
...
.map-search-btn { display: flex; align-items: center; justify-content: center; gap: 4px; padding: 0 20px; background: #3b82f6; color: #fff; border: none; border-radius: 10px; font-weight: bold; cursor: pointer; transition: 0.2s;}
...
.selected-address-bar { background: #f1f5f9; padding: 12px 15px; border-radius: 8px; margin-bottom: 15px; color: #1e293b; font-size: 0.95rem; border-left: 4px solid #3b82f6; font-weight: 500;}
